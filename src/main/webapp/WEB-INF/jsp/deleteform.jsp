...
<html>
    ...
    <body>
        ...
        <!--        Form which has the trainer data to be deleted-->
        <!--Disabled inputs so that no one can overwrite any data -->
            ...
            <form:form method="POST" action="/deleteuser"
                ...
                <c:forEach items="${user}" var="ur">
                    <div style="padding: 2%;">
                        ...
                        <form:input path="userId" placeholder="${ur.userId}" type="hidden" />
                    </div>
                    <div style="padding: 2%;">
                        <form:label path="userName">Name*</form:label>
                        <form:input path="userName" disabled="true"  value="${ur.userName}" class="form-control form-control-sm" type="text" pattern="[a-zA-Z\s]*" name="uname" id="uname" required="required"/>
                    </div>
                    <div style="padding: 2%;">
                        <form:label path="userEmail" >Email*</form:label>
                        <form:input path="userEmail" disabled="true" value="${ur.userEmail}" class="form-control form-control-sm" type="email" name="uemail" id="uemail" required="required" />
                    </div>
                    <div style="padding: 2%;">
                        <form:label path="userCompany" >Company*</form:label>
                        <form:input path="userCompany" disabled="true" value="${ur.userCompany}" class="form-control form-control-sm" type="text" name="ucompany" id="ucompany" required="required"/>
                    </div>


                    <div style="padding: 2%;">
<!--                        Submit button to perform the deletion - controller method /deleteuser-->
                        <span>
                            <button id="myBtn" type="submit" value="Submit" class="w3-button w3-round-xlarge, w3-button w3-pink " 
                                    style="border-radius: 12px;" data_but="btn-xs"><i class='fa fa-paper-plane'></i> Submit</button>
                        </span>
                        <span>
                            <!--                            Button which takes the user back to the deletion list - deletetrainers.jsp-->
                            <td><a href="/deleteusers" class="w3-button w3-round-xlarge,
                                   w3-button w3-pink" style="border-radius: 12px; " 
                                   data_but="btn-xs"><i class='fa fa-reply'></i> Back</a></td>
                        </span>
                    </div>
